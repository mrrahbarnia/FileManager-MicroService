...
async def file_stream_generator_from_s3(file_uuid: str):
    try:
        session = get_session()
        async with session.create_client(
            ...
            response = await client.get_object(
                Bucket=ENVS.S3.BUCKET_NAME, Key=file_uuid
            )  # type: ignore
            async for chunk in response["Body"]:
                yield chunk
    except Exception as ex:
        logger.critical("Getting from S3 bucket failed.", exc_info=ex)
